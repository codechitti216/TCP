...
try:
    ...
    # Provide an explicit error with guidance on how to make the CARLA "agents" package available
    raise RuntimeError(
        "Failed to import CARLA agents package (agents.navigation.*).\n"
        "This typically means the CARLA PythonAPI and/or the 'agents' package are not on PYTHONPATH.\n"
        "Ensure you have CARLA installed and add the following to your environment (example):\n"
        "  export CARLA_ROOT=/path/to/carla && \\n+        export PYTHONPATH=$PYTHONPATH:$CARLA_ROOT/PythonAPI:$CARLA_ROOT/PythonAPI/carla:$CARLA_ROOT/PythonAPI/carla/dist/<egg>\n"
        "Replace <egg> with the actual .egg file present in $CARLA_ROOT/PythonAPI/carla/dist.\n"
        "After setting PYTHONPATH, re-run the script. Original import error: %s" % (e,)
    )
